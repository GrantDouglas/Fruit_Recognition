...
\begin{algorithm} 
    ...
    \For{circle $\in$ circles}{
     ratio = OrangePixelCount(img, circle) \label{alg:CountCircle:line_1}
     
     enoughShowing = CheckOverlap(found, circle)  \label{alg:CountCircle:line_2}
     
    \uIf{$circle.radius > img.size/4 \lor ratio <  20 \lor \neg enoughShowing$}
      {
        continue
      }
...
\begin{algorithm} 
    ...
    \Input{the circle that is being checked $circle$}
    \Input{the binarized image of oranges $img$}
    \Output{percentage of pixels that are orange}
